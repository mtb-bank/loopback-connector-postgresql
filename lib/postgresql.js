...
const debugCasting = require('debug')('loopback:connector:postgresql:casting');
...
PostgreSQL.prototype.count = function (model, where, options, cb) {
  ...
  self.execute(stmt.sql, stmt.params, options, function (err, res) {
    if (err) {
      return cb(err);
    }
 ...
 */
PostgreSQL.prototype.buildColumnNames = function (
  model,
  filter,
  withTable = false
) {
 ...
 */
PostgreSQL.prototype.buildSelect = function (
  model,
  filter,
  options = {},
  count = false
  ...
  let selectStmt = new ParameterizedSQL(
    'SELECT ' + selectSql + ' FROM ' + this.tableEscaped(model)
  );

  if (filter) {
    if (filter.where || filter.order) {
      const { joinStmt, innerWhere, innerOrder } = this.buildJoin(
        model,
        filter
      );
 ...
 */
PostgreSQL.prototype.columnEscaped = function (
  model,
  property,
  withTable = false,
  prefix = ''
...
PostgreSQL.prototype.buildJoin = function (model, filter) {
  ...
  const _buildJoin = (key, type) => {
    ...
    // Keep the candidate properties to handle nested properties
    candidateProperties = [...splitedKey].reverse();
    candidateProperties.pop();
    // Keep the current model property and key
    candidateProperty = candidateProperties[0];
    candidatePropertyKey = candidateProperties[0];
    // Keep the candidate relations
    candidateRelations = splitedKey;
    candidateRelations.pop();
    let parentModel = model;
    let parentPrefix = '';
    let relationPath = model;
    for (let i = 0; i < candidateRelations.length; i++) {
      ...
      if (
        Array.isArray(modelDefinition.settings.__hiddenRelations__) &&
      ...
      if (!(candidateProperty in targetDefinition.properties)) {
        if (
          targetDefinition.settings.__relations__[candidateRelations[i + 1]]
        ) {
          hasProp = false;
        } else {
          let propertyNested = false;
          for (let j = 1; j < candidateProperties.length; j++) {
            if (
              candidateProperties[j] in targetDefinition.properties &&
              targetDefinition.properties[candidateProperties[j]].postgresql &&
              targetDefinition.properties[candidateProperties[j]].postgresql
                .dataType === 'jsonb'
            ) {
              candidateProperty = candidateProperties[j];
              candidateRelations.splice(
                candidateRelations.indexOf(candidateProperty)
              );
              candidatePropertyKey = candidateProperties.reverse().join('.');
              propertyNested = true;
              break;
            }
          }
      ...
      for (const innerJoin of innerJoins) {
        if (potentialRelationPath === innerJoin.relationPath) {
          alreadyJoined = true;
          // Keep the prefix of the found join
          parentPrefix = innerJoin.prefix;
      ...
      if (!alreadyJoined) {
        ...
        const fromProperty = modelDefinition.properties[relation.keyFrom];
        const toProperty = targetDefinition.properties[relation.keyTo];
        const keyFromPropertyType =
          !fromProperty.postgresql || !fromProperty.postgresql.dataType
            ? fromProperty.type.name === 'Number'
              ? 'numeric'
              : 'text'
            : fromProperty.postgresql.dataType;
        const keyToPropertyType =
          !toProperty.postgresql || !toProperty.postgresql.dataType
            ? toProperty.type.name === 'Number'
              ? 'numeric'
              : 'text'
            : toProperty.postgresql.dataType;
        innerJoins.push({
          prefix,
          parentPrefix,
          relationPath,
        ...
        // Keep the prefix for next join
        parentPrefix = prefix;
      }
      // Keep the parentModel for recursive INNER JOIN
  ...
  for (const innerJoin of innerJoins) {
    const isCastingNeeded =
      innerJoin.relation.keyFromPropertyType !==
      innerJoin.relation.keyToPropertyType;
    if (isCastingNeeded) {
      debugCasting(
        'Casting needed for %s (casting refererring key to referenced key)',
        JSON.stringify(innerJoin.relation)
      );
    }
    ...
    joinStmt.sql += `ON ${self.columnEscaped(
      ...
      innerJoin.relation.type === 'belongsTo' && isCastingNeeded
        ? `::${innerJoin.relation.keyToPropertyType}`
        : ''
    } = ${self.columnEscaped(
      innerJoin.model,
      ...
      (innerJoin.relation.type === 'hasOne' ||
        innerJoin.relation.type === 'hasMany') &&
      isCastingNeeded
        ? `::${innerJoin.relation.keyFromPropertyType}`
        : ''
    } `;
...
PostgreSQL.prototype._buildWhere = function (model, filter) {
  ...
  for (const key in where) {
    ...
    if (
      Array.isArray(modelDefinition.settings.__hiddenProps__) &&
